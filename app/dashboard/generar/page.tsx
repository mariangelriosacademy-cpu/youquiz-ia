...
const TIPOS_EXAMEN = [
  { icon: "📝", label: "Examen escrito", desc: "Formato tradicional", config: { cantidad: 20, tipos: ["multiple", "verdadero_falso", "abierta"] } },
  { icon: "✅", label: "Quiz rápido", desc: "10 preguntas rápidas", config: { cantidad: 10, tipos: ["multiple", "verdadero_falso"] } },
  { icon: "🔤", label: "Solo opción múltiple", desc: "Todas de selección", config: { cantidad: 15, tipos: ["multiple"] } },
  { icon: "✔️", label: "Verdadero / Falso", desc: "Solo V/F", config: { cantidad: 15, tipos: ["verdadero_falso"] } },
  { icon: "📖", label: "Preguntas abiertas", desc: "Desarrollo escrito", config: { cantidad: 10, tipos: ["abierta"] } },
  { icon: "🎯", label: "Examen mixto", desc: "Todos los tipos", config: { cantidad: 15, tipos: ["multiple", "verdadero_falso", "abierta"] } },
];
          ...
          <div className="no-print md:hidden mb-4 mt-2">
            <button onClick={() => setMostrarOpciones(!mostrarOpciones)}
              ...
              <span className="flex items-center gap-2">
                ...
                {tipoSeleccionado ? `Tipo: ${tipoSeleccionado}` : "Seleccionar tipo de examen"}
              </span>
              ...
              <div className="mt-2 bg-white/5 border border-white/10 rounded-xl p-3 space-y-1">
                {TIPOS_EXAMEN.map((tipo) => (
                  <button key={tipo.label} onClick={() => aplicarTipoExamen(tipo)}
                    className={`w-full text-left p-2.5 rounded-lg transition flex items-center gap-3 ${
                      tipoSeleccionado === tipo.label ? "bg-violet-600/30 border border-violet-500/40" : "hover:bg-violet-600/20"
                    }`}>
                    <span className="text-lg">{tipo.icon}</span>
                    <div>
                      <p className={`text-sm font-medium ${tipoSeleccionado === tipo.label ? "text-violet-400" : "text-white"}`}>{tipo.label}</p>
                ...
                <div className="pt-2 border-t border-white/10">
                  ...
                  <select value={pais} onChange={(e) => setPais(e.target.value)}
                    className="w-full bg-[#1a1a2e] border border-white/10 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500">
                    {PAISES.map(p => <option key={p.value} value={p.value}>{p.label}</option>)}
                  </select>
                  {paisInfo && <p className="text-xs text-violet-400 mt-1">Escala: {paisInfo.escala} pts</p>}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 min-w-0">
              <div className="no-print mb-4">
                <h1 className="text-lg md:text-xl font-bold text-white">✨ Generar evaluación con IA v2</h1>
              ...
              <div className="no-print flex gap-2 mb-4">
                {[
                  ...
                  { value: "imprimible", icon: <Printer size={14} />, label: "PDF" },
                ].map(m => (
              ...
              <div className="no-print bg-white/5 border border-white/10 rounded-2xl p-4 md:p-5 mb-4">
                <div className="space-y-4">
                  ...
                  <div className="border border-violet-500/20 bg-violet-600/5 rounded-xl p-4 space-y-3">
                    ...
                    <input placeholder="Institución (opcional)" value={institucion}
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-4 md:p-5">
                  ...
                  {mostrarDuplicar && (
                    <div className="no-print mb-4 bg-cyan-600/10 border border-cyan-500/20 rounded-xl p-4">
                      <p className="text-sm font-medium text-cyan-400 mb-3">📋 Duplicar con otro nivel</p>
                      <div className="flex gap-3 items-end flex-wrap">
                        <div className="flex-1 min-w-32">
                          <label className="block text-xs text-slate-400 mb-1">Nivel educativo</label>
                        ...
                        {editandoIdx === i && preguntaEditada ? (
                          <div className="space-y-3">
                            ...
                            <textarea value={preguntaEditada.pregunta}
                              ...
                              rows={2} className="w-full bg-white/5 border border-yellow-500/30 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none" />
                            ...
                            <div className="space-y-1">
                              ...
                              <textarea value={preguntaEditada.explicacion}
                                ...
                                rows={2} className="w-full bg-white/5 border border-white/10 text-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 resize-none" />
                            </div>
                                ...
                                <ul className="space-y-1.5 mb-2">
                                  {p.opciones.map((op, j) => (
                                    <li key={j} className="flex items-center gap-2 text-slate-400 text-xs">
                                      <span className="w-4 h-4 border border-slate-600 rounded flex-shrink-0" />{op}
                                    </li>
                                  ))}
                                </ul>
                              )}
                                ...
                                <div className="flex gap-6 mb-2">
                                  {["Verdadero", "Falso"].map(op => (
                                    <label key={op} className="flex items-center gap-2 text-slate-400 text-xs">
                                      <span className="w-4 h-4 border border-slate-600 rounded-full flex-shrink-0" />{op}
                                    </label>
                                  ))}
                                </div>
                              )}
